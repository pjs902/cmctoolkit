...
def SSE_MS_get_L_and_R(M, Z, t):
    ...
    log_a17 = np.max(
        [
            ...
            np.max([0.097, np.min([0.1461, 0.1461 + 0.1237 * (sigma + 2)])]),
        ]
    )
    a17 = 10**log_a17
    ...
    mu = np.max(
        [
            0.5 * np.ones(len(M)),
            1.0 - 0.01 * np.max([a6 / M**a7, a8 + a9 / M**a10], axis=0),
        ],
        axis=0,
    )
    thook = mu * tBGB
...
def SSE_MS_get_flux(M, Z, t, filttable):
    ...
    for ii in range(len(filtnames)):
        ...
        planck = (
            ...
            * c**2
            / (
                wavelength_cm.reshape((1, wavelength_cm.size)) ** 5
                * (np.exp(h * c / (k * np.outer(Teff_K, wavelength_cm))) - 1)
            )
        )
...
class Snapshot:
    ...
    def make_cuts(
        ...
        if max_lum is not None:
            good = good & (
                (single & (self.data[self.coldict["luminosity_LSUN"]] < max_lum))
                | (
                    binary
                    & (
        ...
        for ii in range(len(filtnames)):
            ...
            planck0 = (
                ...
                * c**2
                / (
                    wavelength_cm.reshape((1, wavelength_cm.size)) ** 5
                    * (np.exp(h * c / (k * np.outer(Teff0_K, wavelength_cm))) - 1)
                )
            )
            ...
            planck1 = (
                ...
                * c**2
                / (
                    wavelength_cm.reshape((1, wavelength_cm.size)) ** 5
                    * (np.exp(h * c / (k * np.outer(Teff1_K, wavelength_cm))) - 1)
                )
            )
    ...
    def make_2d_projection(self, seed=0):
        ...
        vx_arr = (
            vr_arr * sinTheta * np.cos(Phi)
            - r_arr * dotPhi * sinTheta * np.sin(Phi)
            - r_arr * dotTheta * cosTheta * np.cos(Phi)
        )
        vy_arr = (
            vr_arr * sinTheta * np.sin(Phi)
            + r_arr * dotPhi * sinTheta * np.cos(Phi)
        ...
        self.data["x[PC]"], self.data["y[PC]"], self.data["z[PC]"] = (
            x_pc_arr,
            y_pc_arr,
            z_pc_arr,
        )
        self.data["vx[KM/S]"], self.data["vy[KM/S]"], self.data["vz[KM/S]"] = (
            vx_kms_arr,
            vy_kms_arr,
            vz_kms_arr,
        )
    ...
    def make_spatial_density_profile(
        ...
        if bin_edges is None:
            bin_edges = np.logspace(
                np.log10(np.min(self.data["d[PC]"])),
                np.log10(np.max(self.data["d[PC]"])),
                100,
            )
    ...
    def velocity_dispersion(
        ...
        _, veldisp, e_veldisp = self.make_velocity_dispersion_profile(
            bin_edges=[dmin, dmax],
            min_mass=min_mass,
            max_mass=max_mass,
            startypes=startypes,
    ...
    def make_mass_function(
        ...
        if bin_edges is None:
            bin_edges = np.logspace(
                np.log10(np.min(self.data[self.coldict["m_MSUN"]])),
                np.log10(np.max(self.data[self.coldict["m_MSUN"]])),
                100,
            )
        ...
        mass_arr[good & binary & good_bin1] = np.array(
            self.data.loc[good & binary & good_bin1, self.coldict["m1_MSUN"]]
        )
    ...
    def make_smoothed_number_profile(
        self,
        bins=80,
        min_mass=None,
        max_mass=None,
    ...
    def make_smoothed_brightness_profile(
        self,
        filtname,
        bins=80,
        min_mass=None,
    ...
    def make_smoothed_veldisp_profile(
        self,
        bins=80,
        min_mass=None,
        max_mass=None,
        ...
        v_arr = np.array(
            np.hypot(
                self.data.loc[good, self.coldict["vt"]],
                self.data.loc[good, self.coldict["vr"]],
            )
        )
    ...
    def make_paramdict(self):
        ...
        paramdict["sighl_star"] = self.make_smoothed_veldisp_profile(
        ...
        paramdict["sighl_ms"] = self.make_smoothed_veldisp_profile(
            bins=1, dmax=paramdict["rhl"], startypes=startype_ms
        )[1][0]
        ...
        if (
            ...
            paramdict["sighl_ms"] = self.make_smoothed_veldisp_profile(
                bins=1, dmax=paramdict["rhl"], startypes=startype_giant
            )[1][0]
        else:
        ...
        for ii in range(16):
            if paramdict["N_{}".format(ii)] != 0:
                ...
                paramdict["sighl_{}".format(ii)] = self.make_smoothed_veldisp_profile(
                    bins=1, dmax=paramdict["rhl"], startypes=ii
                )[1][0]
            else:
    ...
    def binary_fraction_photometric(
        self,
        filttable,
        mag_filter,
        blue_filter,
        ...
        dmax=None,
        fluxdict=None,
        primary_flux_faint=None,
        primary_flux_brite=None,
        color_pad=0.1,
    ):
        """
        ...
        if min_q is not None:
            ...
            interp_min = scipy.interpolate.interp1d(
                mag_min,
                blue_min - red_min,
                bounds_error=False,
                kind="linear",
            ...
            red_good = self.data.loc[good, f"tot_absMag_{blue_filter}"] - self.data.loc[
        ...
        if max_q is not None:
            ...
            interp_max = scipy.interpolate.interp1d(
                mag_max,
                blue_max - red_max,
                bounds_error=False,
                kind="linear",
